...
namespace sdds {
    ...
    bool Train::load(int& notBoarded) {
        int psgCount = 0;
        int remainingSeats = MAX_NO_OF_PASSENGERS - this->numPassenger;
        std::cout << "Enter number of passengers boarding: " << std::endl << "> ";
        std::cin >> psgCount;
        if (this->numPassenger == MAX_NO_OF_PASSENGERS) {
            notBoarded = psgCount;
            return false;
        }
        if (psgCount > remainingSeats) {
            this->numPassenger += remainingSeats;
            notBoarded = psgCount - remainingSeats;
            return false;
        }
        this->numPassenger += psgCount;
        return true;
    }
